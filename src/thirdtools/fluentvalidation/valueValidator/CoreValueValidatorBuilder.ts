...
export abstract class CoreValueValidatorBuilder<TModel, TValue, TTransformedValue> {
  ...
  public lessThanOrEqualTo = (threshold: number) => {
    const lessThanOrEqualToRule = new LessThanOrEqualToRule<TModel, TTransformedValue>(
      threshold
    );
  ...
  public scalePrecision = (precision: number, scale: number) => {
    ...
    const scalePrecisionRule = new ScalePrecisionRule<TModel, TTransformedValue>(
      precision,
      scale
    );
